...
public abstract class DefaultAdapter<T> extends RecyclerView.Adapter<DefaultHolder<T>> {
    ...
    private DefaultHolder<T> mHolder;
     ...
     */
    @Override
    public DefaultHolder<T> onCreateViewHolder(ViewGroup parent, final int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(getLayoutId(viewType), parent, false);
        mHolder = getHolder(view, viewType);
        mHolder.setOnItemClickListener(new DefaultHolder.OnViewClickListener() {//设置Item点击事件
            @Override
     ...
     */
    @Override
    public void onBindViewHolder(DefaultHolder<T> holder, int position) {
     ...
     * @return
     */
    public abstract DefaultHolder<T> getHolder(View v, int viewType);
    ...
    public static void releaseAllHolder(RecyclerView recyclerView) {
        ...
        for (int i = recyclerView.getChildCount() - 1; i >= 0; i--) {
            ...
            if (viewHolder != null && viewHolder instanceof DefaultHolder) {
                ((DefaultHolder) viewHolder).onRelease();
            }
        }
    }
