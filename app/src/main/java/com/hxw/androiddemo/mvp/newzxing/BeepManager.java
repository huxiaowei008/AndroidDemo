...
/**
 * Manages beeps and vibrations for Activity.
 * 管理哔哔声和震动
 */
final class BeepManager implements MediaPlayer.OnErrorListener, Closeable {
    ...
    private boolean playBeep = true;//打开提示音是否成功
    private boolean vibrate = false;//是否震动(需要权限)
    ...
    synchronized void playBeepSoundAndVibrate() {
        ...
        if (vibrate) {
            if (ContextCompat.checkSelfPermission(activity, Manifest.permission.VIBRATE) !=
                ...
                Toast.makeText(activity, "没有震动权限,请添加", Toast.LENGTH_SHORT).show();
                return;
            }
    ...
    public synchronized boolean onError(MediaPlayer mp, int what, int extra) {
        if (what == MediaPlayer.MEDIA_ERROR_SERVER_DIED) {
            ...
            Toast.makeText(activity, "MEDIA_ERROR_SERVER_DIED", Toast.LENGTH_SHORT).show();
            activity.finish();
        } else {
